"""Repository per mappare apartment_id Smoobu → host_id."""
...
@dataclass
class SmoobuPropertyMapping:
    """Mapping tra apartment_id Smoobu e host_id + property_id interno."""
    
    id: str
    smoobu_apartment_id: int  # Apartment ID Smoobu (es: 101)
...
class SmoobuPropertyMappingsRepository:
    """Repository per gestire mapping Smoobu apartment_id → host_id."""
    
    COLLECTION = "smoobuPropertyMappings"
    
    def __init__(self, client: firestore.Client):
        self._client = client
    
    def get_by_smoobu_apartment_id(
        self, smoobu_apartment_id: int
    ) -> Optional[SmoobuPropertyMapping]:
        """
        Recupera mapping per apartment_id Smoobu.
        
        Args:
            smoobu_apartment_id: Apartment ID Smoobu (es: 101)
        ...
        query = (
            self._collection()
            .where("smoobuApartmentId", "==", smoobu_apartment_id)
            .limit(1)
        )
        docs = list(query.get())
        if not docs:
    ...
    def get_by_host(self, host_id: str) -> list[SmoobuPropertyMapping]:
        """
    ...
    def create_mapping(
        ...
        Args:
            smoobu_apartment_id: Apartment ID Smoobu
        ...
        existing = self.get_by_smoobu_apartment_id(smoobu_apartment_id)
        if existing:
        ...
        data = {
            "smoobuApartmentId": smoobu_apartment_id,
            "hostId": host_id,
        ...
        if internal_property_id:
            data["internalPropertyId"] = internal_property_id
        if property_name:
    ...
    def _deserialize(self, doc: firestore.DocumentSnapshot) -> SmoobuPropertyMapping:
        """Deserializza un documento Firestore in SmoobuPropertyMapping."""
        data = doc.to_dict() or {}
        return SmoobuPropertyMapping(
            id=doc.id,
            smoobu_apartment_id=data.get("smoobuApartmentId", 0),
    ...
    def _collection(self) -> firestore.CollectionReference:
        """Restituisce la collection reference."""
